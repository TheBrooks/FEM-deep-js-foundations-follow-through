function initUI() {
	...
	let handleClick = function submitWorkEntry(){
...
function addWorkToProject(projectId,description,minutes) {
	...
	if (projectEntryData.work.length > 1) {
		...
		projectEntryData.work = projectEntryData.work.slice().sort(function descendingSort(a,b){
...
function addWorkEntryToList(projectEntryData,workEntryData) {
	...
	if (projectEntryData.work.length > 1) {
		...
		for (let entryIdx = 0; entryIdx < projectEntryData.work.length; entryIdx++) {
